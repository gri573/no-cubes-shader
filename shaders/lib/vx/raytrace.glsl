...
//round tracing experiment

void doLighting(inout vec3 col, vec3 pos, vec3 normal) {
    col *= 0.1 + max(dot(normal, sunDir), 0);
}
...
vec3 ipw(vec3 x) {
    return vec3(ipw(x.x), ipw(x.y), ipw(x.z));
}
vec4 roundTrace(inout vec3 pos0, inout vec3 dir, sampler2D atlas) {
    int initDir = 0;
    ...
    vec3 normal;
    float totalInt;
    vec4 adjPos = vec4(-1000);
    int k;
    #define STEP_LIMIT 200
    for (k = 0; raycolor.a < 0.98 && k < STEP_LIMIT; k++) {
        pos = pos0 + w * dir;
        if (!isInRange(pos) || w > 1) {
            pos0 = vec3(-10000);
            dir = vec3(0);
            return raycolor;
        }
        ivec2 coords = getVxPixelCoords(pos);
        ivec4 data1 = ivec4(texelFetch(shadowcolor1, coords, 0) * 65535 + 0.5);
        if (true ||data1.w < 2) {
            vec3 floorOrigPos = floor(pos);
            vec3 floorPos = floorOrigPos;
            ivec4[8] data;
                for (int j = 0; j < 8; j++) {
                ivec3 offset = ivec3(j % 2, (j >> 1) % 2, (j >> 2) % 2);
                vec3 nextPos = floorPos + offset + 0.1;
                if (isInRange(nextPos)) {
                    data[j] = ivec4(texelFetch(shadowcolor0, getVxPixelCoords(nextPos), 0) * 65535 + 0.5);
                } else data[j] = ivec4(65535);
            }
            for (;k < STEP_LIMIT && length(abs(floorPos - floorOrigPos)) < 0.1; k++) {
                vec3 intPolFactor = ipw(pos - floorPos);
                vec3[3] adjIntPolFactor = vec3[3](
                    ipw(pos - floorPos + vec3(0.01, 0, 0)),
                    ipw(pos - floorPos + vec3(0, 0.01, 0)),
                    ipw(pos - floorPos + vec3(0, 0, 0.01))
                );
                float density = 0;
                normal = vec3(0);
                adjPos = vec4(-1000);
                for (int j = 0; j < 8; j++) {
                    if (data[j].w == 65535) continue;
                    ivec3 offset = ivec3(j % 2, (j >> 1) % 2, (j >> 2) % 2);
                    vec3 nextPos = floorPos + offset + 0.1;
                    vec3 intMult0 = offset + (1 - 2 * offset) * intPolFactor;
                    float intMult = intMult0.x * intMult0.y * intMult0.z;
                    vec4 adjPos0;
                    adjPos0.xyz = floorPos + offset;
                    adjPos0.a = -length(pos - adjPos0.xyz);
                    if (adjPos.a < adjPos0.a) {
                        adjPos = adjPos0;
                    }
                    density += intMult;
                    for (int i = 0; i < 3; i++) {
                        vec3 adjIntMult0 = offset + (1 - 2 * offset) * adjIntPolFactor[i];
                        float adjIntMult = adjIntMult0.x * adjIntMult0.y * adjIntMult0.z;
                        normal[i] += adjIntMult - intMult;
                    }
                }
                if (initDir == 0) {
                    initDir = 1 - 2 * int(density > 0.5);
                    if (abs(density - 0.5) < 0.1) w += 0.01 * invDirLen;
                }
                normal /= -initDir * (length(normal) + 0.00001);
                float stepSize = (1.5 + 0.5 * dot(normal, dir) * invDirLen) * initDir * (0.4 - density) * invDirLen;
                w += stepSize;
                pos = pos0 + w * dir;
                floorPos = floor(pos);
                if (abs(stepSize) <= 0.02 * min(invDirLen, 0.1 * w)) {
                    if (initDir == -1) {
                        raycolor += (1 - raycolor.a) * (1 - exp((1 / (1.01 - lastcol.a) - 1) * (lastw - w))) * lastcol;
                    }
                    vec3 vxPos = adjPos.xyz;//floor(pos + 0.4 * normal + 0.5);
                    vxData hitBlockData = readVxMap(getVxPixelCoords(vxPos + 0.5));
                    vec3 dPos = pos - vxPos;
                    int n;
                    float maxNormal = 0;
                    for (int i = 0; i < 3; i++) {
                        if (abs(normal[i]) > maxNormal) {
                            n = i;
                            maxNormal = abs(normal[i]);
                        }
                    }
                    vec2 spriteCoord = 1.96 * vec2(dPos[(n+1)%3], dPos[(n+2)%3]);
                    spriteCoord = clamp(spriteCoord, vec2(-0.98), vec2(0.98));
                    vec4 hitCol = texelFetch(atlas, ivec2(atlasSize * hitBlockData.texcoord + hitBlockData.spritesize * spriteCoord), 0);
                    hitCol.rgb *= hitCol.rgb;
                    hitCol.rgb *= hitBlockData.lightcol;
                    lastcol = hitCol;
                    lastw = w;
                    doLighting(hitCol.rgb, pos, normal);
                    raycolor += (1 - raycolor.a) * hitCol * hitCol.a;
                    initDir *= -1;
                    w += 0.1 * invDirLen;
                    break;
                }
            }
        } else w += 0.7 * invDirLen * (data1.w - 1);
    }
    pos0 = pos;
    dir = normal;
    return raycolor;
